...
const poison = pokémon.filter( p => p.types.length < 2 & p.types.includes("poison")).map(p => p.name);
// console.log(poison)

// an array containing just the first type of all the pokémon whose second type is "flying"
const typefly = pokémon.filter( p => p.types.length > 1 & p.types[1] === "poison").map( p => p.types[0]);
// console.log(typefly)
